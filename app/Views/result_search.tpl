...
{if isset($result['avis'])}
      ...
      {foreach from=$result['avis'] item=item key=key}
            <div class="card"  style="width: 25rem;">
                  <img class="card-img-top" src="{base_url('assets/Images/avis')|cat:'/'|cat:$item['image']}" alt="Card image cap">
                  <div class="card-body">
                        <h5 class="card-title">{$item['avis']}</h5>
                        <p class="card-text">{$item['contenue']}</p>
                  </div>
                  <div class="card-body">
                        <a href="{base_url('avis')}" class="card-link btn btn-primary">Go</a>
                  </div>
            </div>
      {/foreach}
{/if}
